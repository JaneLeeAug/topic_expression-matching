...
const readInterface = readline.createInterface({
    input: fs.createReadStream('D:/Trie_input.txt'),
    output: process.stdout,
    console: false
});
// 先把目標寫入檔清空
const removeAll = '';
fs.writeFile('D:/Trie_output.txt', removeAll, function (err) {
    if (err) throw err;
});
fs.writeFile('D:/Trie_outputTest.txt', removeAll, function (err) {
    if (err) throw err;
});
// 開始讀
let integer = 0;
...
readInterface.on('line', function (line) {
    ...
    fsPromises.appendFile('D:/Trie_output.txt', str3);
    fsPromises.appendFile('D:/Trie_outputTest.txt', str6)
    // 重置line值，準備讀取下一行字串
    line = undefined;
    cou++;
    if (cou === N) {
